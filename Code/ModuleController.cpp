...
update_status ModuleController::PreUpdate() {
	...
	for (int i = 0; i < numjoystick; i++) {
		...
		buttonstate[i][4] = SDL_GameControllerGetButton(controller[i], SDL_CONTROLLER_BUTTON_DPAD_UP);
		buttonstate[i][5] = SDL_GameControllerGetButton(controller[i], SDL_CONTROLLER_BUTTON_DPAD_DOWN);
		buttonstate[i][6] = SDL_GameControllerGetButton(controller[i], SDL_CONTROLLER_BUTTON_DPAD_LEFT);
		buttonstate[i][7] = SDL_GameControllerGetButton(controller[i], SDL_CONTROLLER_BUTTON_DPAD_RIGHT);
		buttonstate[i][8] = SDL_GameControllerGetButton(controller[i], SDL_CONTROLLER_BUTTON_BACK);
		buttonstate[i][9] = SDL_GameControllerGetButton(controller[i], SDL_CONTROLLER_BUTTON_START);
		buttonstate[i][10] = SDL_GameControllerGetButton(controller[i], SDL_CONTROLLER_BUTTON_RIGHTSHOULDER);
		buttonstate[i][11] = SDL_GameControllerGetButton(controller[i], SDL_CONTROLLER_BUTTON_RIGHTSHOULDER);
