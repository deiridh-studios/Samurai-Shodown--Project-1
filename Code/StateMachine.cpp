...
Animation* ExecuteState(bool &movementextra, bool &sword, bool pow, bool &notfinished, int& jump_timer, int& punch_timer, int& kick_timer, int& specialattack_timer, int& hitted_timer, state actual, bool flip, int speed,  int& mult, bool stopright, bool stopleft, bool& aftercrouch, Collider& body, Collider& body2, Collider& body3, Collider **attack, iPoint& position, Module* Player, Animation& current) {
	...
	case A_PUNCH_STANDING:
		if (sword == true) {
			if (Player == App->player) {
				current_animation = &App->player->strongerslash;
				checkanim = App->player->strongerslash;
			}
			else {
				current_animation = &App->player2->strongerslash;
				checkanim = App->player2->strongerslash;
			}
		}
		...
		if (punch_timer == 1) {
			...
			if (flip == false) {
				...
				if (Player == App->player) *attack = App->collision->AddCollider({ position.x - 34,(position.y - 75),75,50 }, COLLIDER_PLAYER_SHOT, App->player);
				else if (Player == App->player2) *attack = App->collision->AddCollider({ position.x - 39,(position.y - 75),75,50 }, COLLIDER_ENEMY_SHOT, App->player2);
			}
		}
		checkanim.GetCurrentFrame();
		if (flip == true && (int)checkanim.current_frame == 4 && movementextra == false) {
			position.x = position.x - 50;
			movementextra = true;
		}
		else if (flip == true && movementextra == true&& (int)checkanim.current_frame != 4) {
			position.x = position.x + 50;
			movementextra = false;
		}
	...
	case A_PUNCHL_STANDING:
		if (sword == true) {
			if (Player == App->player) {
				...
				checkanim=App->player->punch;
			}
			else {
				current_animation = &App->player2->punch;
				checkanim= App->player2->punch;
			}
		}
		else {
		...
		if (punch_timer == 1) {
			...
			if (flip == false) {
				...
				if (Player == App->player) *attack = App->collision->AddCollider({ position.x -44,(position.y - 75),75,50 }, COLLIDER_PLAYER_SHOT, App->player);
				else if (Player == App->player2) *attack = App->collision->AddCollider({ position.x -49,(position.y - 75),75,50 }, COLLIDER_ENEMY_SHOT, App->player2);
			}				
		}
		checkanim.GetCurrentFrame();
		if (flip == true && (int)checkanim.current_frame == 3 && movementextra == false) {
			position.x = position.x - 30;
			movementextra = true;
		}
		else if(flip == true && (int)checkanim.current_frame == 4 && movementextra == true) {
			position.x = position.x - 20;
			movementextra = false;
		}
		else if (flip == true && (int)checkanim.current_frame == 5 && movementextra == false) {
			position.x = position.x + 20;
			movementextra = true;
		}
		else if (flip == true && movementextra == true && (int)checkanim.current_frame >5 ) {
			position.x = position.x + 30;
			movementextra = false;
		}
		if (current_animation->GetFinished() == 1) {
	...
	case A_PUNCHS_STANDING:
		if (sword = true) {
			if (Player == App->player) {
				current_animation = &App->player->powerfulslash;
				checkanim = App->player->powerfulslash;
			}
			else {
				current_animation = &App->player2->powerfulslash;
				checkanim = App->player2->powerfulslash;
			}
		}
		else { current_animation = &App->player->strongpunch_noweapon; }
		checkanim.GetCurrentFrame();
		if (punch_timer == 1) {
			App->audio->PlayChunk(App->player->strongattacksound);
			punch_timer = 2;
			if (flip == false) {
				body.SetPos(position.x + 38, (position.y - 80));
				body2.SetPos(position.x + 44, (position.y - 57));
				body3.SetPos(position.x + 30, (position.y - 40));
				if (Player == App->player) *attack = App->collision->AddCollider({ position.x + 83,(position.y - 155),75,125 }, COLLIDER_PLAYER_SHOT, App->player);
				else if (Player == App->player2) *attack = App->collision->AddCollider({ position.x + 115,(position.y - 155),75,125 }, COLLIDER_ENEMY_SHOT, App->player2);
			}
			else {
				body.SetPos(position.x - 5, (position.y - 70));
				body2.SetPos(position.x - 12, (position.y - 54));
				body3.SetPos(position.x - 12, (position.y - 40));
				if (Player == App->player) *attack = App->collision->AddCollider({ position.x - 49,(position.y - 155),75,125 }, COLLIDER_PLAYER_SHOT, App->player);
				else if (Player == App->player2) *attack = App->collision->AddCollider({ position.x - 54,(position.y - 155),75,125 }, COLLIDER_ENEMY_SHOT, App->player2);
			}				//COLLIDER BOX*/
		}
		if (flip == true && (int)checkanim.current_frame == 2 && movementextra == false) {
			position.x = position.x - 70;
			movementextra = true;
		}
		else if (flip == true && movementextra == true && (int)checkanim.current_frame>6) {
			position.x = position.x + 70;
			movementextra = false;
		}
		if (current_animation->GetFinished() == 1) {
	...
	case A_PUNCHS_NEUTRAL_JUMP:
		...
		if (sword = true) {
			if (Player == App->player) {
				...
				checkanim = App->player->punchair;
			}
			else {
				current_animation = &App->player2->jump;
				current_animation->GetCurrentFrame();
				current_animation = &App->player2->punchair;
				checkanim = App->player2->punchair;
			}
		}
		...
		if (punch_timer == 1) {
			...
			punch_timer = 2;
			if (flip == false) {
				if (Player == App->player) *attack = App->collision->AddCollider({ position.x + 69,(position.y - 70),75,40 }, COLLIDER_PLAYER_SHOT, App->player);
				else if (Player == App->player2) *attack = App->collision->AddCollider({ position.x + 50,(position.y - 70),75,40 }, COLLIDER_ENEMY_SHOT, App->player2);
			}
			else {
				if (Player == App->player) *attack = App->collision->AddCollider({ position.x - 85,(position.y - 70),75,40 }, COLLIDER_PLAYER_SHOT, App->player);
				else if (Player == App->player2) *attack = App->collision->AddCollider({ position.x - 75,(position.y - 70),75,40 }, COLLIDER_ENEMY_SHOT, App->player2);
			}
		}
		if (flip == true && (int)checkanim.current_frame == 3 && movementextra == false) {
			position.x = position.x - 70;
		...
		if (punch_timer != 1) {
			if(Player == App->player)(*attack)->SetPos(position.x + 69, (position.y - 70));
			else if (movementextra == false) (*attack)->SetPos(position.x - 85, (position.y - 70));
			else (*attack)->SetPos(position.x - 15, (position.y - 70));
		}
		if (current_animation->GetFinished() == 1) {
			punch_timer = 3;
			(*attack)->to_delete = true;
	...
	case A_PUNCH_CROUCH:
		if (sword = true) {
			if (Player == App->player)current_animation = &App->player->punchcrouch;
			else current_animation = &App->player2->punchcrouch;
		}
		else {
		...
		if (punch_timer == 1) {
			...
			punch_timer = 2;
			if (flip == false) {
				if (Player == App->player) *attack = App->collision->AddCollider({ position.x + 76,(position.y - 60),75,30 }, COLLIDER_PLAYER_SHOT, App->player);
				else if (Player == App->player2) *attack = App->collision->AddCollider({ position.x + 76,(position.y - 60),75,30 }, COLLIDER_ENEMY_SHOT, App->player2);
			}
			else {
				if (Player == App->player) *attack = App->collision->AddCollider({ position.x + 1,(position.y - 60),75,30 }, COLLIDER_PLAYER_SHOT, App->player);
				else if (Player == App->player2) *attack = App->collision->AddCollider({ position.x + 1,(position.y - 60),75,30 }, COLLIDER_ENEMY_SHOT, App->player2);
				position.x -= 80;
			}
		}
		if (current_animation->GetFinished() == 1) {
	...
	case A_PUNCHL_CROUCH:
		...
		if (punch_timer == 1) {
			...
			punch_timer = 2;
			if (flip == false) {
				if (Player == App->player) *attack = App->collision->AddCollider({ position.x + 76,(position.y - 60),75,40 }, COLLIDER_PLAYER_SHOT, App->player);
				else if (Player == App->player2) *attack = App->collision->AddCollider({ position.x + 76,(position.y - 60),75,40 }, COLLIDER_ENEMY_SHOT, App->player2);
				position.x += 24;
			}
			else {
				if (Player == App->player) *attack = App->collision->AddCollider({ position.x + 1,(position.y - 60),75,40 }, COLLIDER_PLAYER_SHOT, App->player);
				else if (Player == App->player2) *attack = App->collision->AddCollider({ position.x + 1,(position.y - 60),75,40 }, COLLIDER_ENEMY_SHOT, App->player2);
				position.x -= 24;
			}			//COLLIDER BOX*/
		}
		if (current_animation->GetFinished() == 1) {
	...
	case A_KICK_STANDING:
		if (sword = true) {
			if (Player == App->player)current_animation = &App->player->kick;
			else current_animation = &App->player2->kick;
		}//ANIMATION
		...
		if (kick_timer == 1) {
			...
			kick_timer = 2;
			if (flip == false) {
				if (Player == App->player) *attack = App->collision->AddCollider({ position.x + 66,(position.y - 48),50,98 }, COLLIDER_PLAYER_SHOT, App->player);
				else if (Player == App->player2) *attack = App->collision->AddCollider({ position.x + 66,(position.y - 90),65,20 }, COLLIDER_ENEMY_SHOT, App->player2);
			}
			else {
				if (Player == App->player) *attack = App->collision->AddCollider({ position.x -20 ,(position.y - 48),50,98 }, COLLIDER_PLAYER_SHOT, App->player);
				else if (Player == App->player2) *attack = App->collision->AddCollider({ position.x -20,(position.y - 48),50,98 }, COLLIDER_ENEMY_SHOT, App->player2);
			}
		}
		if (current_animation->GetFinished() == 1) {
			kick_timer = 3;
	...
	case A_KICKL_STANDING:
		...
		if (kick_timer == 1) {
			...
			if (flip == false) {
				if (Player == App->player) *attack = App->collision->AddCollider({ position.x + 66,(position.y - 48),50,98 }, COLLIDER_PLAYER_SHOT, App->player);
				else if (Player == App->player2) *attack = App->collision->AddCollider({ position.x +66,(position.y - 48),50,98 }, COLLIDER_ENEMY_SHOT, App->player2);
				position.x += 24;
			}
			else {
				if (Player == App->player) *attack = App->collision->AddCollider({ position.x -20 ,(position.y - 48),50,98 }, COLLIDER_PLAYER_SHOT, App->player);
	...
	case A_KICKS_STANDING:
		if (sword = true) {
			if (Player == App->player)current_animation = &App->player->strongerkick;
			else current_animation = &App->player2->strongerkick;
		}//ANIMATION
		else { current_animation = &App->player->kick_noweapon;  }//ANIMATION
		if (flip == false) {
			body.SetPos(position.x + 50, (position.y - 20));
	...
	case A_KICK_NEUTRAL_JUMP:
		if (sword = true) {
			if (Player == App->player)current_animation = &App->player->kickair;
			else current_animation = &App->player2->kickair;
		}//ANIMATION
		...
		if (kick_timer == 1) {
			...
			kick_timer = 2;
			if (flip == false) {
				if (Player == App->player) *attack = App->collision->AddCollider({ position.x + 40,(position.y - 28),65,20 }, COLLIDER_PLAYER_SHOT, App->player);
				...
				if (Player == App->player) *attack = App->collision->AddCollider({ position.x + 4,(position.y - 28),65,20 }, COLLIDER_PLAYER_SHOT, App->player);
	...
	case A_KICK_CROUCH:
		if (sword = true) {
			if (Player == App->player)current_animation = &App->player->kickcrouch;
			else current_animation = &App->player2->kickcrouch;
		}
		else {
	...
	case A_KICKL_CROUCH:
		...
		if (kick_timer == 1) {
			...
			kick_timer = 2;
			if (flip == false) {
				if (Player == App->player) *attack = App->collision->AddCollider({ position.x + 30,(position.y - 30),75,30 }, COLLIDER_PLAYER_SHOT, App->player);
				else if (Player == App->player2) *attack = App->collision->AddCollider({ position.x + 30,(position.y - 30),75,30 }, COLLIDER_ENEMY_SHOT, App->player2);
				position.x += 24;
			}
			else {
	...
	case A_HITTED:
		if (sword = true) {
			if (Player == App->player)current_animation = &App->player->hittedan;
			else current_animation = &App->player2->hittedan;
		}
		else {
	...
	case A_HITTED_JUMP:
		if (Player == App->player)current_animation = &App->player->airhitted;
		else current_animation = &App->player2->airhitted;
		if (hitted_timer == 1 /*&& position.y == 210*/ && Player == App->player) {
			App->audio->PlayChunk(App->player->hittedsound);
			if (pow == false && App->player2->pow == false) {
				if (App->player2->actual3 == A_PUNCH_STANDING) App->UI->DamageTaken(1, ((float)32 / (float)7), 3);
	...
	case A_APPLEATTACK:
		...
		if (specialattack_timer == 1) {
			...
			position.y += 30;
			if (flip == false) {
				if (Player == App->player) *attack = App->collision->AddCollider({ position.x + 75,(position.y - 130),75,200 }, COLLIDER_PLAYER_SHOT, App->player);
				else if (Player == App->player2) *attack = App->collision->AddCollider({ position.x + 75,(position.y - 130),75,200 }, COLLIDER_ENEMY_SHOT, App->player2);
			}
			if (flip == true) {
				if (Player == App->player) *attack = App->collision->AddCollider({ position.x - 49,(position.y - 130),75,30 }, COLLIDER_PLAYER_SHOT, App->player);
				else if (Player == App->player2) *attack = App->collision->AddCollider({ position.x - 49,(position.y - 130),75,30 }, COLLIDER_ENEMY_SHOT, App->player2);
				position.x -= 15;
			}
		}
		if (current_animation->GetFinished() == 1) {
	...
	case A_DASH:
		if (sword == true) {
			if (Player == App->player)current_animation = &App->player->dash;
			else current_animation = &App->player2->dash;
		}
		else {
		...
		else specialattack_timer = 3;
		if (flip == false) {
			body.SetPos(position.x + 50, (position.y - 80));
			body2.SetPos(position.x + 52, (position.y - 57));
			body3.SetPos(position.x + 45, (position.y - 40));
		}
		else {
			body.SetPos(position.x + 50, (position.y - 80));
			body2.SetPos(position.x + 52, (position.y - 57));
		...
		if (current_animation->GetFinished() == 1 || (stopright == true || stopleft == true)) {
			specialattack_timer = 3;
		}
		break;
