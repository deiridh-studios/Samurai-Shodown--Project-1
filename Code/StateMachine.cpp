...
Animation* ExecuteState(bool &movementextra, bool &sword, bool pow, bool &notfinished, int& jump_timer, int& punch_timer, int& kick_timer, int& specialattack_timer, int& hitted_timer, state actual, bool flip, int speed,  int& mult, bool stopright, bool stopleft, bool& aftercrouch, Collider& body, Collider& body2, Collider& body3, Collider **attack, iPoint& position, Module* Player, Animation& current) {
	...
	/*case A_ROLLATTACK:
		...
		if(sword == true)current_animation = &App->player->rollattack;
		else { break; }
		App->audio->PlayChunk(App->player->tornadosound);
		if (flip == false) {
		...
		if (specialattack_timer == 1) {
			...
			specialattack_timer = 2;
			if (flip == false) {
				if (Player == App->player) *attack = App->collision->AddCollider({ position.x + 80,(position.y - 115),105,210 }, COLLIDER_PLAYER_SHOT, App->player);
				else if (Player == App->player2) *attack = App->collision->AddCollider({ position.x + 80,(position.y - 115),105,210 }, COLLIDER_ENEMY_SHOT, App->player2);
			}
			else {
